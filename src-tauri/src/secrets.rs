...
const TAVILY_ACCOUNT: &str = "tavily:api_key";

pub fn save_tavily_api_key(api_key: &str) -> Result<(), String> {
    let entry = keyring::Entry::new(SERVICE, TAVILY_ACCOUNT)
        .map_err(|error| format!("Cannot open system credential store: {error}"))?;
    entry
        .set_password(api_key)
        .map_err(|error| format!("Cannot save Tavily API key: {error}"))
}
...
pub fn load_tavily_api_key() -> Result<Option<String>, String> {
    ...
    match entry.get_password() {
        ...
        Err(keyring::Error::NoEntry) => Ok(None),
        Err(error) => Err(format!(
            "Cannot read Tavily API key: {error}"
        )),
    }
}

pub fn delete_tavily_api_key() -> Result<(), String> {
    let entry = keyring::Entry::new(SERVICE, TAVILY_ACCOUNT)
        .map_err(|error| format!("Cannot open system credential store: {error}"))?;
    match entry.delete_password() {
        Ok(()) | Err(keyring::Error::NoEntry) => Ok(()),
        Err(error) => Err(format!(
            "Cannot delete Tavily API key: {error}"
        )),
    }
}
